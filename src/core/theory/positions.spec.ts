...
import { chordPositions, findPosition, isInPosition, POSITION_SPAN } from './positions'
...
describe('chordPositions', () => {
  it('C 的把位錨在 5 弦第 3 格與 6 弦第 8 格（開放 C 與 E 型 barre）', () => {
    ...
    expect(positions.map((p) => `${p.rootString}@${p.rootFret}`)).toEqual([
      '5@3', '6@8', '5@15', '6@20',
    ])
  ...
  it('D 的最低把位錨在 4 弦空弦（開放 D 是 D 型指型）', () => {
    ...
    expect(positions[0]).toMatchObject({ rootString: 4, rootFret: 0, fromFret: 0, toFret: 4 })
    expect(positions[1]).toMatchObject({ rootString: 5, rootFret: 5 })
  })

  it('同起始格時低音弦優先（6 弦指型比 4 弦指型常用）', () => {
    // E：6 弦空弦與 4 弦第 2 格都會落在第 0 格起
    const positions = chordPositions(4)
    expect(positions[0]).toMatchObject({ rootString: 6, rootFret: 0 })
    expect(positions.some((p) => p.rootString === 4 && p.rootFret === 2)).toBe(false)
  })
  ...
  it('每個把位都框得住根音，且寬度不超過 span（開放把位除外）', () => {
    for (const key of ALL_KEYS) {
      for (const p of chordPositions(parseNoteName(key).pc)) {
        expect(isInPosition(p, p.rootFret), `${key} ${p.id} 沒框到根音`).toBe(true)
        if (p.fromFret > 0) {
