 ...
 * 琶音示範音的接線（七和弦琶音模組）。
 *
 * 與 useChordDemo 的差別只有一件事，但那件事就是琶音的定義：
 ...
 * 在十六分細分下那是聽得出來的。
 *
 * 「這一格該彈哪個音」不在這一層——那是練習設計（modules/chords/arpeggio/sequence.ts）。
 ...
 */
export function useArpeggioDemo(pitchAt: (e: TickEvent) => number | undefined): void {
  const transport = useTransportStore()

  const unsubscribe = transport.subscribeSchedule((e) => {
    if (transport.chordDemo === 'off') return
    // 靜默小節（示範／靜默循環）連琶音也不發：那一段就是要你自己撐住
