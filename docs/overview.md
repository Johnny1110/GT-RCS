...
- **音階模進（Phase 7 之後追加）**：即興的素材不是音階本身，是**模進**——同一個音型往上搬一格再一格。
  指型系統由音階決定：七音音階是**一弦三音**（7 個指型），五聲是**盒型**（5 個，每弦兩音）；
  模進型由「一組幾個音 × 組內跳幾個音階級數」兩個正交參數長出八種（三個一組、四個一組、
  三度／四度跳音、三和弦與七和弦分解…），方向可選上行／下行／上下行。
  一格一個音跟著 click 走，指板上的白圈標出**確切那一格**（路徑給的是格位，不是音名）。
  組長度除不盡小節時序列會跨過小節線，畫面直接說出「幾個小節之後回到開頭」——
  那是爵士與 fusion 長句子的張力來源，不說出來會被當成壞掉。
