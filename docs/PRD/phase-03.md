...
7. **和弦指板加上把位框**（上線後依使用者回饋補強）：全指板 40 幾個和弦音看不出指型，
   等於看不出把位。把位由 `core/theory/positions.ts` 推導——以**低音弦上的根音**錨定
   （6 弦＝E 型、5 弦＝A 型、4 弦＝D 型，這就是吉他指型的命名方式），
   每個把位往高把位涵蓋 5 格；根音在第 3 格以內時琴枕當 barre，把位往下延伸到空弦。
   候選框依起始格排序、同格時低音弦優先，貪婪地丟掉重疊者，因此**框保證互不重疊**。
   框外的音點不刪除、只降透明度——它們確實是和弦音，只是不屬於任何一個好按的指型。
   測試以 12 調 × 5 種和弦性質窮舉驗證「每個框內都湊得齊全部和弦音」。
   音階把位是另一套系統（3-notes-per-string 與五聲盒型，且把位彼此重疊），
   由 `scalePositions()` 另行推導、以 `positionMode='focus'` 呈現——見 phase-02 決策 6。
