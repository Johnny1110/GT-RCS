...
## 4. 驗收標準（Phase DoD）
- [ ] 五度圈互動：12 大調＋12 小調點選，diatonic highlight 全部正確（測試背書）
- [ ] 進行引擎：預設進行在 12 調展開的和弦名稱與品質全部正確（快照測試）
- [ ] 跟練：和弦切換、五度圈脈衝、指板更新三者與 click 小節數嚴格同步；「下一個和弦」提前一小節出現
- [ ] 12 調循環完整走完並寫入練習日誌
- [ ] 6 個預設進行 + 5 級課表 + 知識內容（zh-TW/en）上線
- [ ] 指板同步更新無可感知延遲（和弦切換 < 1 frame）
